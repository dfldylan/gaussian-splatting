    ...
    while True:
        ...
        try:
            ...
            if custom_cam != None:
                ...
                gaussians_frame: GaussianModel = gaussians_list[frame]
                if mask is not None:
                    gaussians_frame = copy.deepcopy(gaussians_frame)
                    gaussians_frame.prune_points(mask)
                net_image = render(custom_cam, gaussians_frame, pipe, background, scaling_modifer)["render"]
...
if __name__ == "__main__":
    ...
    pipeline(lp.extract(args), op.extract(args), pp.extract(args))
